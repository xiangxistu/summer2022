...
rt_uint32_t pwm1_period = 1000;
rt_uint32_t pwm1_pulse = 500;

rt_uint32_t pwm2_period = 1000;
rt_uint32_t pwm2_pulse = 500;
...
rt_err_t my_pwm_set_pulse(struct rt_device_pwm * pwm_dev,rt_uint32_t pulse_val)
{
    ...
    RT_ASSERT(pwm_dev);
    if(pwm_dev == pwm1)
    ret = rt_pwm_set(pwm_dev, 1, PWM_PERIOD, pulse_val);
    if(pwm_dev == pwm2)
    ret = rt_pwm_set(pwm_dev, 2, PWM_PERIOD, pulse_val);
    return ret;
}
